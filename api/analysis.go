...
func (h AnalysisHandler) AppDeps(ctx *gin.Context) {
	...
	filter, err := qf.New(ctx,
		...
		Field{Name: "version", Kind: STRING},
		Field{Name: "type", Kind: STRING},
		Field{Name: "indirect", Kind: STRING},
		Field{Name: "sha", Kind: STRING})
	if err != nil {
		_ = ctx.Error(err)
		return
	}
...
//    ?filter=category=(info|warn)
//    ?filter=effort=20
//    ?filter=application.id=(1|2|3)
//    ?filter=tag.id=(1|2|3)
//    ?filter=technology.source=hibernate
//    ?filter=technology.target=hibernate
